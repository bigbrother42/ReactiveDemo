...
namespace ServiceDemo.Common.SQLite
{
    public class NoteService : ClientApiService
    {
        ...
        public async Task<List<NoteCategoryCustomWebDto>> SelectQuickSearchMatchedListAsync(string keyWord)
        {
            var noteCategoryList = new List<NoteCategoryCustomWebDto>();
            ...
            var taskResult = await Task.Run(() =>
            {
                ...
                {
                    sql += $" AND (nt.TypeName LIKE '%{keyWord}%' OR nc.CategoryName LIKE '%{keyWord}%' OR nc2.Content LIKE '%{keyWord}%')";
                }
